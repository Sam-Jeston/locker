use database::channels::{get_channels_for_client, create_channel};
use database::models::{ToJsonForm, ChannelJson};
    ...
    fn on_message(&mut self, msg: ws::Message) -> ws::Result<()> {
        ...
        match serde_json::from_str::<ChannelBody>(raw_msg) {
            Ok(parsed_msg) => {
                ...
                let channels_json: Vec<ChannelJson> = channels.into_iter().map(|c| c.to_json_form()).collect();
